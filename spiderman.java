...
public class spiderman {
    ...
    public static void main(String[] args) {
        ...
        while(t-->0) {
            ...
            //Add an edge with "infinite" flow from the source to the museum
            din.add(din.s, 0, 100000, 0);
            //Add an edge with "infinite" flow from the getaway location to the sink
            din.add(n - 1, din.t, 100000, 0);
            for (int i = 0; i < m; i++) {
                int a = scan.nextInt() - 1;
                int b = scan.nextInt() - 1;
                //Add an edge with 1 flow between each intersection that has a road connecting them.
                din.add(a, b, 1, 0);
                din.add(b, a, 1, 0);
            }
            //Calculate max flow
            int flow = din.flow();
            //If the max flow is equal to 1, that means we can place a blockade to limit the flow to 0
            if (flow <= 1) {
                System.out.println("Halt, Spider-Man! Plans Thwarted!");
            } else {
                System.out.println("How Sad, Perpetrator Triumphed.");
            }
        }
    }
    static class Dinic {
